...
class StepViewerMethodsMixin:
    ...
    def handle_panel_clicked(self, info):
        ...
        self.clicked_panel_text.setPlainText(
            ...
            f"Layer: {info.get('panel_layer', '-')}\n"
            f"Center: ({info['cx'] * factor:.1f}, {info['cy'] * factor:.1f}, {info['cz'] * factor:.1f}) {unit}\n"
            f"Size: {info['x'] * factor:.1f} × {info['y'] * factor:.1f} × {info['z'] * factor:.1f} {unit}"
        )
        self.status_label.setText(f"Clicked panel: {panel_name}")
    ...
    def load_step(self, file_path: str):
        try:
            ...
            polydata = self._cadquery_shape_to_vtk_polydata(shape)
    ...
    def _surface_rect_points(
        ...
        zf = z * factor
        return [
            (xmin * factor, ymin * factor, zf),  # A
            (xmax * factor, ymin * factor, zf),  # B
            (xmax * factor, ymax * factor, zf),  # C
            (xmin * factor, ymax * factor, zf),  # D
        ]
    ...
    def _surface_intersection_info(
        ...
        return {
            ...
            "area": area,
            "ratio": ratio,
            "points": self._surface_rect_points(rect, top_panel["zmax"], factor=1.0),
        }
    ...
    def build_intersected_panels_report(self) -> str:
        ...
        if groups:
            ...
            for group in groups:
                ...
                lines.append(
                    f"{chain_label} : Surface Points (A,B,C,D)"
                )
                ...
                lines.append(
                    f"size: {ix:.1f} × {iy:.1f} {unit} | area: {area_scaled:.1f} {unit}²"
                )
                lines.append("")
        ...
        for top in sorted(infos, key=lambda x: x.get("panel_order", 99999)):
            ...
            if below_items:
                ...
                for bottom, info in below_items:
                    pts = self._surface_rect_points(info["rect"], top["zmax"], factor=factor)
                    lines.append(
                        f"  - {bottom['panel_name']} "
                        f"(overlap {info['ratio'] * 100:.1f}%)"
                    )
                    lines.append(
